...
impl CloudflareCacheManager {
    ...
    fn get_token(&self) -> Result<String> {
        env::var("CF_API_TOKEN")
            .context("CF_API_TOKEN environment variable not set")
    }
    ...
    fn purge_via_cli(&self, zone_id: Option<&str>) -> Result<CloudflarePurgeResult> {
        ...
        let message = if success {
            ...
            format!("Cloudflare CLI failed: {}", String::from_utf8_lossy(&output.stderr))
        };
    ...
    fn purge_via_api(&self, zone_id: Option<&str>) -> Result<CloudflarePurgeResult> {
        ...
        if let Some(zone) = zone_id {
            ...
            let url = format!("https://api.cloudflare.com/client/v4/zones/{}/purge_cache", zone);
            ...
            let message = if success {
                ...
                format!("Cloudflare API failed: {}", String::from_utf8_lossy(&output.stderr))
            };
            ...
            Ok(CloudflarePurgeResult {
                success: false,
                message: "Zone ID required for Cloudflare API purge. Use --zone <id> or wrangler CLI.".to_string(),
                zone_id: None,
                timestamp: Utc::now(),
                method: "api".to_string(),
            })
