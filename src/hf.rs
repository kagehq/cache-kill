...
impl HfCacheManager {
    ...
    pub fn get_stats(&self) -> Result<HfStats> {
        ...
        let mut repo_sizes: std::collections::HashMap<String, u64> = std::collections::HashMap::new();
        let mut model_sizes: std::collections::HashMap<String, u64> = std::collections::HashMap::new();
    ...
    if config.json {
        ...
        if !stats.top_repos.is_empty() {
            ...
            for (repo, size) in &stats.top_repos[..5] {
                println!("  {}: {}", repo, humansize::format_size(*size, humansize::DECIMAL));
            }
        }
        ...
        if !stats.top_models.is_empty() {
            ...
            for (model, size) in &stats.top_models[..5] {
                println!("  {}: {}", model, humansize::format_size(*size, humansize::DECIMAL));
            }
        }
    }
...
mod tests {
    ...
    fn test_parse_hf_path() {
        ...
        let path = Path::new("/home/user/.cache/huggingface/hub/models--microsoft--DialoGPT-medium");
