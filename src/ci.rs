...
impl CiCacheManager {
    ...
    pub fn execute(&self) -> Result<CiResult> {
        ...
        // Discover caches
        let discovery =
            DiscoveryResult::discover(&self.config).context("Failed to discover caches")?;
        ...
        let entries = inspector
            .inspect_caches(&discovery.cache_entries)
        ...
        let (freed_bytes, backup_dir) = match self.mode {
            ...
            CiMode::Postbuild => {
                ...
                if self.config.dry_run {
                    ...
                    (
                        result.total_size,
                        Some(result.backup_dir.to_string_lossy().to_string()),
                    )
                }
            }
        };

        let status = if freed_bytes > 0 {
            "success"
        } else {
            "no_action"
        };
    ...
    fn create_summary(
        ...
        summary.insert(
            "freed_human".to_string(),
            humansize::format_size(freed_bytes, humansize::DECIMAL),
        );
    ...
    pub fn print_summary(&self, result: &CiResult, json_mode: bool) -> Result<()> {
        if json_mode {
            ...
            println!(
                "CACHEKILL_CI: mode={:?} entries={} freed={} status={}",
